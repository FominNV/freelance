...
import {
  RrTableColumnImpl,
  RrTableCellStylePropImpl,
  RrTableChangeSelectionEventImpl,
  RrTableComtextMenuItemImpl,
} from './interface';
import { ClassPropsType } from '../../@types/props';

export interface RrTablePropsImpl {
  /** высота ячейки */
  ...
  cellStyle?: RrTableCellStylePropImpl;
   ...
   */
  contextMenu?: Array<RrTableComtextMenuItemImpl>;
...
const emit = defineEmits<{
  ...
  /** клик по ячейке */
  (e: 'cell-click', row: any, column: RrTableColumnImpl): void;
  /** смена состояния checkbox/radio */
  (e: 'change:selection', event: RrTableChangeSelectionEventImpl): void;
...
const props = withDefaults(defineProps<RrTablePropsImpl>(), {
  cellHeight: 44,
  columns: () => [],
  items: () => [],
  theme: 'default-view',
...
const preparedColumns = computed<RrTableColumnImpl[]>(() => {
  return props.columns.map((col) => ({
    ...col,
    // TODO: Типизировать any
    renderBodyCell: ({ row, column, rowIndex }: any, h: CreateElement) => {
    ...
    // TODO: Типизировать any
    renderHeaderCell: ({ column }: any, h: CreateElement) => {
...
const cellStyleOption = computed<RrTableCellStylePropImpl | null>(() => {
  if (props.cellStyle) {
    return {
      ...props.cellStyle,
    };
...
const expandOption = computed(() => ({
  ...
  render: ({ row, column, rowIndex }: any, h: CreateElement) => {
