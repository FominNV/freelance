...
import {
  RrPaginationDefaultPropsImpl,
  RrPaginationPagesListItemImpl,
  RrPaginationDefaulEmitsImpl,
  RrPaginationPageSizesListItemImpl,
} from './interface';

const PagesListBuilder = (count: ComputedRef<number>, currentPage: ComputedRef<number>) => computed<Array<RrPaginationPagesListItemImpl>>(() => {
  const result: Array<RrPaginationPagesListItemImpl> = [];
...
const PageSizesListBuilder = (pageSizes: number[], pageSize: ComputedRef<number>) => computed<RrPaginationPageSizesListItemImpl[]>(() => {
...
export function usePagination(data: RrPaginationDefaultPropsImpl, emit: RrPaginationDefaulEmitsImpl) {
