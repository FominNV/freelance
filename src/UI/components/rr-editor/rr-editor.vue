...
export interface RrEditorPropsImpl {
  /** отключить редактор */
  disabled?: boolean;
  ...
  /** параметры редактора */
  editorSettings?: RrEditorSettings;
  /** модули редактора */
  customModules?: Array<RrEditorCustomModulesImpl>;
}

updateToolbarIcons();

const props = withDefaults(defineProps<RrEditorPropsImpl>(), {
  customModules: () => [],
...
const emit = defineEmits<{
  ...
  (e: 'selection-change', event: (range: RrEditorRangeImpl, oldRange: RrEditorRangeImpl, source: string) => void): void;
